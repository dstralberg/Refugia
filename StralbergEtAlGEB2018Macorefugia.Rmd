...
a. A key component of the index is a distance-decay function based on a fat-tailed dispersal kernel, 
...
b. The following code loops through species, calculates study area prevalence (mean probability of occurrence), 
...
c. This core bit of code calculates biotic velocity (Carroll et al. 2015) for each species based on the 
...
a. The next step is to calculate the multi-species refugia index. The following code reads in refugia index layers 
...
a. The following code can be used to calculate local topographic metrics (landform proportions) from an land facet layer 
...
b. The following code can be used to calculate landscape topographic metrics from an elevation layer 
(here at 10-km resolution). 

```{landscape topographic metrics}
tpi<-terrain(elev,opt='tpi',neighbors=8)
rough<-terrain(elev,opt='roughness',neighbors=8)
```

c. This code can be used to calculate regional topographic position indices from an elevation layer 
(here at 10-km resolution). 

```{regional topographic position indices}
...
d. And here is code to calculate the north-south corridor index from a 10-km DEM

```{north-south corridor index}
...
a. Combine continental/topographic and future climate layers in a raster stack and crops them to match the refugia
layer extent.

```{raster stack of topographic and climate variables}
...
b. Extract environmental data to a regular sample of points covering approximately 10% of the study region.

```{extract environmental data to sample points}
set.seed(999)
...
c. The following code can be used to build and analyze quantile regression models (Cade and Noon 2003) for nested and separate models
...
a. The final portions of code can be used to conduct a limiting factor analysis, sensu Greenberg et al.(2015)
...
b. Next, generate scatterplots for each set of univariate relationships, develop and predict univariate regression models
...
```{univariate quantile regression scatterplots}
...
c. Finally, calculate limiting factors for each pixel based on bootstrap resampling of quantile regression models,
and generate raster stacks containing limiting factors (variables yielding minimum predictions) for each quantile value

```{bootstrapped quantile regression models}
...
d. Use the Mode and Mode_count functions, defined below (from Lucas Fortini on stackoverflow.com), to calcuate the most
frequent limiting factor variable (mode), and the frequency of that variable (mode_count).

```{mode and frequency of limiting factors}
Mode <- function(x) { 
  ux <- unique(x)
  ux=ux[!is.na(ux)]
